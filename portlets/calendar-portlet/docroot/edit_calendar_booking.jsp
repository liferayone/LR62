...
<aui:form action="<%= updateCalendarBookingURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "updateCalendarBooking();" %>'>
	...
	<aui:fieldset>
		<aui:input  defaultLanguageId="<%= themeDisplay.getLanguageId() %>" name="title" showRequiredLabel="<%= false %>">
			<aui:validator name="required"/>
		</aui:input>

		<div class="<%= allDay ? "allday-class-active" : "" %>" id="<portlet:namespace />startDateContainer">
			<aui:input ignoreRequestValue="<%= true %>" label="start-date-uat" name="startTime" value="<%= startTimeJCalendar %>" />
		</div>

		<div class="<%= allDay ? "allday-class-active" : "" %>" id="<portlet:namespace />endDateContainer">
			<aui:input ignoreRequestValue="<%= true %>" label="end-date-uat" name="endTime" value="<%= endTimeJCalendar %>" />
		</div>
	...
	<aui:fieldset>
		<liferay-ui:panel-container extended="<%= true %>" id="calendarBookingDetailsPanelContainer" persistState="<%= true %>">
			<liferay-ui:panel collapsible="<%= true %>" defaultState="closed" extended="<%= false %>" id="calendarBookingDetailsPanel" persistState="<%= true %>" title="details">
				<aui:select style="display:none;" label="" name="calendarId">
					...
					for (Calendar curCalendar : manageableCalendars) {
						...
						//Restricting the DropDown List to only 12 Capital
						if(HtmlUtil.escape(curCalendar.getName(locale)).equalsIgnoreCase("Twelve Capital")){
					%>
				...
				<aui:input type="text" name="location" showRequiredLabel="<%= false %>">
	...
	<aui:button-row>
		...
		<c:if test="<%= calendarBooking != null %>">
			...
			/>
			 ...
			 <aui:button style="display:none;" href="<%= permissionsCalendarBookingURL %>" value="permissions" /> 
		</c:if>
	</aui:button-row>
</aui:form>
