...
List<UserGroup> userGroupList= UserGroupLocalServiceUtil.getUserUserGroups(themeDisplay.getUser().getUserId());
boolean showAddEventBtn=false;
boolean showPopOverButtons= true;
boolean disableEventBox=true;
List<Role> userRoles = user.getRoles();
for(Role r : userRoles){
if("Administrator".equalsIgnoreCase(r.getName()) || "Site Administrator".equalsIgnoreCase(r.getName()) || "Event Author".equalsIgnoreCase(r.getName()) ){
	showAddEventBtn=true; 
	readOnly= ParamUtil.getBoolean(request, "readOnly");
	showPopOverButtons=false;
	disableEventBox=false;
}
}

for(UserGroup usersGroup: userGroupList){
	if(RoleLocalServiceUtil.hasGroupRole(usersGroup.getGroup().getGroupId(), RoleLocalServiceUtil.getRole(themeDisplay.getCompanyId(), "Event Author").getRoleId())){
		showPopOverButtons=false;
		showAddEventBtn=true; 
		disableEventBox=false; 
		readOnly= ParamUtil.getBoolean(request, "readOnly");
...
<% if(showPopOverButtons){ 
	
%>
<style type="text/css">

 .popover-footer #saveBtn{
 	display: none !important;
 }
 .popover-footer #editBtn{
 	display: none !important;
 }
 .popover-footer #deleteBtn{
 	display: none !important;
 }
